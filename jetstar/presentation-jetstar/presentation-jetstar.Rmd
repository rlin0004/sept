---
title: "Jetstar Case Study Interview"
subtitle: "A focus on Improving the NPS score"  
author: 
  - "Author: Ruimin Lin"
output:
...
location_promoter <- nps_location %>%
  filter(Q1.1._NPS_GROUP == "Promoter")

location_detractor <- nps_location %>%
  filter(Q1.1._NPS_GROUP == "Detractor")

location_passive <- nps_location %>%
  filter(Q1.1._NPS_GROUP == "Passive")

nps_location1 <- nps_location %>%
  group_by(DEPARTURE_CITY,
           ARRIVAL_CITY) %>%
  summarise(n = sum(n)) %>%
  ungroup() %>%
  mutate(location_score = location_promoter$pct - location_detractor$pct)
```

```{r}
p2a <- nps_location1 %>%
  ggplot(aes(x = DEPARTURE_CITY,
             y = location_score,
             colour = DEPARTURE_CITY)) +
  geom_point(aes(size = location_score,
                 alpha = location_score)) +
  scale_size_continuous(range = c(3, 11)) +
  scale_alpha_continuous(range = c(0.3, 1)) +
  xlab("Departure City") +
  labs(title = "NPS per Location") +
  theme_minimal() +
...
p2b <- perth1 %>%
  ...
  scale_y_continuous(labels = scales::percent) +
  theme_minimal() +
  scale_fill_manual(values = colourD) +
...
◾ Capture key NPS trends

--

◾ Discover the underlying driver of NPS movements

--

◾ Discover possible areas to improve NPS
...
```{r}
t1 <- tibble(
  Variable = c("ResponseId", "Q1.1._NPS_Group", "Q2.1._9 ~ Q2.1._12",
               "DATE_OF_TRAVEL", "DAY_OF_TRAVEL", "MONTH_OF_TRAVEL",
               "FARE_COST", "DEPARTURE_DELAY"),
  Description = c("Unique Identifier",
                  "NPS groups: Promoter, Passives, and Detractor",
                  "Total of 9 survey questions on customer satisfaction, score of 0-10.",
                  "Date of travel",
                  "Day of Travel extracted from DATE_OF_TRAVEL",
                  "Month of Travel extracted from DATE_OF_TRAVEL",
                  "Cost of fare in AUD,USD, NZD, and HKD",
                  "Delay from scheduled time of departure (in minutes)")
)

t1 %>%
  kable(caption = "Variable description for Jetstar NPS Data") %>%
...
## NPS per Departure City

.panelset[
   .panel[.panel-name[Figure 1]

```{r departure, echo=FALSE, out.width="80%"}
knitr::include_graphics("departureplot.png")
```
   ]
   .panel[.panel-name[Figure 2]
   
```{r delay, echo=FALSE, out.width="80%"}
knitr::include_graphics("delayplot.png")
```
...
```{r corplot, echo=FALSE, out.width="80%"}
...
## Explore on satisfaction survey questions
...
```{r rplot, echo=FALSE}
knitr::include_graphics("responseplot.png")
```
...
```{r satplot, echo=FALSE}
knitr::include_graphics("satplot.png")
``` 
...
### NPS and CSAT Key Trend & Findings

◾ Strong positive relationship between NPS and CSAT score.

◾ Perth and Darwin as Departure location tends to have low NPS scores compare to other locations.

◾ Both NPS and CSAT are negatively affected by Departure Delay and Fare Cost.

◾ Around 3/4 customers responds to the satisfaction survey questions (except question about on-board service).

### Recommended Areas to imrpove

◾ Improve customer experience on: On board service, purchases made on-board.

◾ Improve on timeliness of flight, especially on departure city like Perth, Darwin, etc.
...
◾ Collecting customer demographics for further analysis if possible.
